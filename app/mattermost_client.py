...
class MattermostClient:
    ...
    def __init__(self):
        ...
        if self.api_token:
            ...
            logger.warning(f"Mattermost API token not configured - thread support will be limited")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.debug(f"Post data retrieved: root_id={post_data.get('root_id')}, parent_id={post_data.get('parent_id')}")
                    return post_data
                else:
                    logger.warning(
